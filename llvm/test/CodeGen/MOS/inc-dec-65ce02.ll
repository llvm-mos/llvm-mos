...
; RUN: llc -mcpu=mos4510 -verify-machineinstrs < %s | FileCheck %s
...
; --- Chained INW/DEW for small constant pointer offsets (65CE02+) ---
; Pointer offsets up to +/-2 chain into INW/DEW; +/-3 and beyond fall back to
; a generic 16-bit ADC, which is faster than a longer chain. Integers are
; unchanged: only +/-1 becomes INC/DEC, as before.
...
; Verify i16 -=2 falls back to generic ADC: the decrement threshold is 1
; because each chained DEC would need its own cmp #255 to detect the borrow.
...
define ptr @inc_ptr_by3(ptr %p) {
; CHECK-LABEL: inc_ptr_by3:
; CHECK:       ; %bb.0: ; %entry
...
; CHECK-NEXT:    rts
entry:
  %0 = getelementptr inbounds i8, ptr %p, i16 3
  ret ptr %0
}
...
define ptr @dec_ptr_by3(ptr %p) {
; CHECK-LABEL: dec_ptr_by3:
; CHECK:       ; %bb.0: ; %entry
; CHECK-NEXT:    lda __rc2
; CHECK-NEXT:    clc
; CHECK-NEXT:    adc #253
