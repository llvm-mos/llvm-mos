; RUN: llvm-mc -assemble --print-imm-hex --show-encoding -triple mos --mcpu=mos45gs02 < %s | FileCheck %s
	...
	; TODO: Enable checks
	aslq $abcd, x				; _CHECK: encoding: [0x42,0x42,0x1e,0xab,0xcd]
	rolq $abcd, x				; _CHECK: encoding: [0x42,0x42,0x3e,0xab,0xcd]
	lsrq $abcd, x				; _CHECK: encoding: [0x42,0x42,0x5e,0xab,0xcd]
	rorq $abcd, x				; _CHECK: encoding: [0x42,0x42,0x7e,0xab,0xcd]
	ldq $abcd, x				; _CHECK: encoding: [0x42,0x42,0xbd,0xab,0xcd]
	deq $abcd, x				; _CHECK: encoding: [0x42,0x42,0xde,0xab,0xcd]
	inq $abcd, x				; _CHECK: encoding: [0x42,0x42,0xfe,0xab,0xcd]
