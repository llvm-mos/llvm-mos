...
SystemZTargetLowering::SystemZTargetLowering(const TargetMachine &TM,
  ...
  for (MVT VT : MVT::integer_fixedlen_vector_valuetypes()) {
    if (isTypeLegal(VT)) {
      ...
      if (Subtarget.hasVectorEnhancements3() && VT != MVT::v16i8 &&
          VT != MVT::v8i16) {
...
static void adjustICmp128(SelectionDAG &DAG, const SDLoc &DL, Comparison &C) {
  ...
  if ((C.CCMask == SystemZ::CCMASK_CMP_EQ ||
    ...
    if (Src.hasOneUse()) {
      ...
      case SystemZISD::VICMPE:
        Opcode = SystemZISD::VICMPES;
        break;
      case SystemZISD::VICMPH:
        Opcode = SystemZISD::VICMPHS;
        break;
      case SystemZISD::VICMPHL:
        Opcode = SystemZISD::VICMPHLS;
        break;
      case SystemZISD::VFCMPE:
        Opcode = SystemZISD::VFCMPES;
        break;
      case SystemZISD::VFCMPH:
        Opcode = SystemZISD::VFCMPHS;
        break;
      case SystemZISD::VFCMPHE:
        Opcode = SystemZISD::VFCMPHES;
        break;
      default:
        break;
...
static SDValue emitCmp(SelectionDAG &DAG, const SDLoc &DL, Comparison &C) {
  ...
  if (C.Opcode == SystemZISD::VICMPES || C.Opcode == SystemZISD::VICMPHS ||
      C.Opcode == SystemZISD::VICMPHLS || C.Opcode == SystemZISD::VFCMPES ||
      C.Opcode == SystemZISD::VFCMPHS || C.Opcode == SystemZISD::VFCMPHES) {
...
static SDValue getI128Select(SelectionDAG &DAG, const SDLoc &DL, Comparison C,
                             SDValue TrueOp, SDValue FalseOp) {
  EVT VT = MVT::i128;
  unsigned Op;
...
SDValue SystemZTargetLowering::lowerSELECT_CC(SDValue Op,
  ...
  if (Subtarget.hasVectorEnhancements3() && C.Opcode == SystemZISD::ICMP &&
      C.Op0.getValueType() == MVT::i128 && TrueOp.getValueType() == MVT::i128) {
...
SDValue SystemZTargetLowering::lowerMULH(SDValue Op, SelectionDAG &DAG,
  ...
  lowerGR128Binary(DAG, DL, VT, Opcode, Op.getOperand(0), Op.getOperand(1),
                   Even, Odd);
  return Even;
}
...
static bool isAddCarryChain(SDValue Carry) {
  ...
  return Carry.getOpcode() == ISD::UADDO && Carry->getValueType(0) != MVT::i128;
}
...
static bool isSubBorrowChain(SDValue Carry) {
  while (Carry.getOpcode() == ISD::USUBO_CARRY &&
         Carry->getValueType(0) != MVT::i128)
    Carry = Carry.getOperand(2);
  return Carry.getOpcode() == ISD::USUBO && Carry->getValueType(0) != MVT::i128;
}
...
SystemZTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::s390_vmaeg:
    return DAG.getNode(ISD::ADD, SDLoc(Op), Op.getValueType(),
                       DAG.getNode(SystemZISD::VME, SDLoc(Op),
                                   Op.getValueType(), Op.getOperand(1),
                                   Op.getOperand(2)),
                       Op.getOperand(3));
  ...
  case Intrinsic::s390_vmaleg:
    return DAG.getNode(ISD::ADD, SDLoc(Op), Op.getValueType(),
                       DAG.getNode(SystemZISD::VMLE, SDLoc(Op),
                                   Op.getValueType(), Op.getOperand(1),
                                   Op.getOperand(2)),
                       Op.getOperand(3));
  ...
  case Intrinsic::s390_vmaog:
    return DAG.getNode(ISD::ADD, SDLoc(Op), Op.getValueType(),
                       DAG.getNode(SystemZISD::VMO, SDLoc(Op),
                                   Op.getValueType(), Op.getOperand(1),
                                   Op.getOperand(2)),
                       Op.getOperand(3));
  ...
  case Intrinsic::s390_vmalog:
    return DAG.getNode(ISD::ADD, SDLoc(Op), Op.getValueType(),
                       DAG.getNode(SystemZISD::VMLO, SDLoc(Op),
                                   Op.getValueType(), Op.getOperand(1),
                                   Op.getOperand(2)),
                       Op.getOperand(3));
  }
...
SystemZTargetLowering::combineSIGN_EXTEND_INREG(SDNode *N,
  ...
  if (EVT == MVT::i1 && N0.hasOneUse() && N0.getOpcode() == ISD::SETCC) {
    ...
    SDValue Ops[] = {N0.getOperand(0), N0.getOperand(1),
                     DAG.getAllOnesConstant(DL, VT), DAG.getConstant(0, DL, VT),
                     N0.getOperand(2)};
...
SDValue SystemZTargetLowering::combineSETCC(SDNode *N,
                                            DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue
SystemZTargetLowering::combineShiftToMulAddHigh(SDNode *N,
                                                DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
  SDLoc DL(N);
  ...
  SDValue Result =
      DAG.getNode(IsSignExt ? SystemZISD::VMAH : SystemZISD::VMALH, DL,
                  NarrowVT, LeftOp.getOperand(0), MulhRightOp, MulhAddOp);
...
SDValue SystemZTargetLowering::combineMUL(SDNode *N,
                                          DAGCombinerInfo &DCI) const {
  SelectionDAG &DAG = DCI.DAG;
...
SDValue SystemZTargetLowering::PerformDAGCombine(SDNode *N,
  ...
  case ISD::SRL:
  case ISD::SRA:
    return combineShiftToMulAddHigh(N, DCI);
  case ISD::MUL:
