...
protected:
  bool processBlock(MachineBasicBlock &MBB) {
    ...
    for (MachineBasicBlock::iterator I = MBB.begin(), IE = MBB.end();
      ...
      bool IsTLSLDAIXMI =
          (MI.getOpcode() == PPC::TLSLDAIX8 || MI.getOpcode() == PPC::TLSLDAIX);

      if (MI.getOpcode() != PPC::ADDItlsgdLADDR &&
          MI.getOpcode() != PPC::ADDItlsldLADDR &&
          MI.getOpcode() != PPC::ADDItlsgdLADDR32 &&
          MI.getOpcode() != PPC::ADDItlsldLADDR32 &&
          MI.getOpcode() != PPC::TLSGDAIX && MI.getOpcode() != PPC::TLSGDAIX8 &&
          !IsTLSTPRelMI && !IsPCREL && !IsTLSLDAIXMI) {
      ...
      if (IsAIX) {
        if (IsTLSLDAIXMI) {
          ...
          unsigned LDTocOp = Is64Bit
                                 ? (IsLargeModel ? PPC::LDtocL : PPC::LDtoc)
              ...
              for (MachineOperand &MO : UseIter->operands())
                if (MO.isReg() && MO.isUse()) {
                  ...
                  if (RegInfo.hasOneDef(MOReg)) {
                    MachineInstr *Temp = RegInfo.getOneDef(MOReg)->getParent();
          ...
          BuildMI(MBB, I, DL, TII->get(TargetOpcode::COPY), GPR3).addReg(InReg);
