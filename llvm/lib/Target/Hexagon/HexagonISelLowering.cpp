...
HexagonTargetLowering::LowerToTLSGeneralDynamicModel(GlobalAddressSDNode *GA,
  ...
  return GetDynamicTLSAddr(DAG, Chain, GA, InGlue, PtrVT, Hexagon::R0, Flags);
}
...
const char *HexagonTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  case HexagonISD::MFSHL:
    return "HexagonISD::MFSHL";
  case HexagonISD::MFSHR:
...
bool HexagonTargetLowering::validateConstPtrAlignment(SDValue Ptr,
                                                      Align NeedAlign,
                                                      const SDLoc &dl,
                                                      SelectionDAG &DAG) const {
...
SDValue HexagonTargetLowering::replaceMemWithUndef(SDValue Op,
                                                   SelectionDAG &DAG) const {
  const SDLoc &dl(Op);
...
SDValue HexagonTargetLowering::getSplatValue(SDValue Op,
                                             SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
...
SDValue HexagonTargetLowering::extractVectorPred(SDValue VecV, SDValue IdxV,
                                                 const SDLoc &dl, MVT ValTy,
                                                 MVT ResTy,
                                                 SelectionDAG &DAG) const {
...
SDValue HexagonTargetLowering::insertVectorPred(SDValue VecV, SDValue ValV,
                                                SDValue IdxV, const SDLoc &dl,
                                                MVT ValTy,
                                                SelectionDAG &DAG) const {
  MVT VecTy = ty(VecV);
...
SDValue HexagonTargetLowering::getCombine(SDValue Hi, SDValue Lo,
...
SDValue HexagonTargetLowering::LowerLoad(SDValue Op, SelectionDAG &DAG) const {
  ...
  bool LoadPred =
      MemTy == MVT::v2i1 || MemTy == MVT::v4i1 || MemTy == MVT::v8i1;
  if (LoadPred) {
...
SDValue HexagonTargetLowering::LowerOperation(SDValue Op,
  ...
  case ISD::USUBO_CARRY:
    return LowerUAddSubOCarry(Op, DAG);
  case ISD::SRA:
  case ISD::SHL:
  case ISD::SRL:
