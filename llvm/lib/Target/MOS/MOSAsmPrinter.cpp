...
void MOSAsmPrinter::printOperand(const MachineInstr *MI, unsigned OpNo,
                                 raw_ostream &O, const char *Modifier) {
  const MachineOperand &MO = MI->getOperand(OpNo);

  switch (MO.getType()) {
  case MachineOperand::MO_Register:
    O << MOSInstPrinter::getPrettyRegisterName(MO.getReg(), MRI);
    break;
...
bool MOSAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned OpNum,
                                    unsigned AsmVariant, const char *ExtraCode,
                                    raw_ostream &O) {
...
bool MOSAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
  ...
  // for registers.
  if (MI->getOperand(OpNum).getReg() == MOS::R31R30) {
    O << "Z";
  } else {
    assert(MI->getOperand(OpNum).getReg() == MOS::R29R28 &&
