...
MOSTargetLowering::MOSTargetLowering(const MOSTargetMachine &TM,
  ...
  addRegisterClass(MVT::i8, &MOS::GPR8RegClass);
  addRegisterClass(MVT::i16, &MOS::DREGSRegClass);
  ...
  // Do not use MUL. The MOS instructions are closer to SMUL_LOHI &co.
...
EVT MOSTargetLowering::getSetCCResultType(const DataLayout &DL, LLVMContext &,
                                          EVT VT) const {
  assert(!VT.isVector() && "No MOS SetCC type for vectors!");
  return MVT::i8;
}

SDValue MOSTargetLowering::LowerShifts(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (!isa<ConstantSDNode>(N->getOperand(1))) {
    ...
    case ISD::SHL:
      return DAG.getNode(MOSISD::LSLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRL:
      return DAG.getNode(MOSISD::LSRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTL:
      return DAG.getNode(MOSISD::ROLLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::ROTR:
      return DAG.getNode(MOSISD::RORLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    case ISD::SRA:
      return DAG.getNode(MOSISD::ASRLOOP, dl, VT, N->getOperand(0),
                         N->getOperand(1));
    }
  }
...
SDValue MOSTargetLowering::LowerDivRem(SDValue Op, SelectionDAG &DAG) const {
...
SDValue MOSTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(MOSISD::WRAPPER, SDLoc(Op), getPointerTy(DL), Result);
}
...
/// IntCCToMOSCC - Convert a DAG integer condition code to an MOS CC.
static MOSCC::CondCodes intCCToMOSCC(ISD::CondCode CC) {
  switch (CC) {
  default:
...
/// Returns appropriate MOS CMP/CMPC nodes and corresponding condition code for
/// the given operands.
SDValue MOSTargetLowering::getMOSCmp(SDValue LHS, SDValue RHS, ISD::CondCode CC,
                                     SDValue &MOScc, SelectionDAG &DAG,
                                     SDLoc DL) const {
  SDValue Cmp;
  ...
  case ISD::SETGT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case -1: {
        ...
        UseTest = true;
        MOScc = DAG.getConstant(MOSCC::COND_PL, DL, MVT::i8);
        break;
      }
      case 0: {
  ...
  case ISD::SETLT: {
    if (const ConstantSDNode *C = dyn_cast<ConstantSDNode>(RHS)) {
      ...
      case 0: {
        ...
        UseTest = true;
        MOScc = DAG.getConstant(MOSCC::COND_MI, DL, MVT::i8);
        break;
      }
      }
    }
  ...
  if (VT == MVT::i32) {
    ...
    if (UseTest) {
      ...
      SDValue Top = DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8, LHShi,
                                DAG.getIntPtrConstant(1, DL));
      Cmp = DAG.getNode(MOSISD::TST, DL, MVT::Glue, Top);
    } else {
      Cmp = DAG.getNode(MOSISD::CMP, DL, MVT::Glue, LHSlo, RHSlo);
      Cmp = DAG.getNode(MOSISD::CMPC, DL, MVT::Glue, LHShi, RHShi, Cmp);
    }
  } else if (VT == MVT::i64) {
    ...
    if (UseTest) {
      ...
      Cmp = DAG.getNode(MOSISD::CMP, DL, MVT::Glue, LHS0, RHS0);
      Cmp = DAG.getNode(MOSISD::CMPC, DL, MVT::Glue, LHS1, RHS1, Cmp);
      Cmp = DAG.getNode(MOSISD::CMPC, DL, MVT::Glue, LHS2, RHS2, Cmp);
      Cmp = DAG.getNode(MOSISD::CMPC, DL, MVT::Glue, LHS3, RHS3, Cmp);
    }
  } else if (VT == MVT::i8 || VT == MVT::i16) {
    if (UseTest) {
      // When using tst we only care about the highest part.
      Cmp = DAG.getNode(MOSISD::TST, DL, MVT::Glue,
                        (VT == MVT::i8)
                            ? LHS
                            : DAG.getNode(ISD::EXTRACT_ELEMENT, DL, MVT::i8,
                                          LHS, DAG.getIntPtrConstant(1, DL)));
    } else {
      Cmp = DAG.getNode(MOSISD::CMP, DL, MVT::Glue, LHS, RHS);
    }
  } else {
    llvm_unreachable("Invalid comparison size");
  }

  // When using a test instruction MOScc is already set.
  if (!UseTest) {
    MOScc = DAG.getConstant(intCCToMOSCC(CC), DL, MVT::i8);
  }

  return Cmp;
}

SDValue MOSTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue TargetCC;
  SDValue Cmp = getMOSCmp(LHS, RHS, CC, TargetCC, DAG, dl);

  return DAG.getNode(MOSISD::BRCOND, dl, MVT::Other, Chain, Dest, TargetCC,
                     Cmp);
}

SDValue MOSTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(MOSISD::SELECT_CC, dl, VTs, Ops);
}

SDValue MOSTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
...
SDValue MOSTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
...
SDValue MOSTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  default:
...
bool MOSTargetLowering::isLegalAddressingMode(const DataLayout &DL,
  ...
  if (isa<PointerType>(Ty) && AS == MOS::ProgramMemory) {
    return false;
  }
...
bool MOSTargetLowering::getPreIndexedAddressParts(SDNode *N, SDValue &Base,
                                                  SDValue &Offset,
  ...
  if (const LoadSDNode *LD = dyn_cast<LoadSDNode>(N)) {
    ...
    if (LD->getExtensionType() != ISD::NON_EXTLOAD)
      return false;
    if (MOS::isProgramMemoryAccess(LD)) {
      return false;
    }
  } else if (const StoreSDNode *ST = dyn_cast<StoreSDNode>(N)) {
    VT = ST->getMemoryVT();
    Op = ST->getBasePtr().getNode();
    if (MOS::isProgramMemoryAccess(ST)) {
      return false;
    }
  } else {
    return false;
...
bool MOSTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                   SDValue &Base,
                                                   SDValue &Offset,
                                                   ISD::MemIndexedMode &AM,
                                                   SelectionDAG &DAG) const {
...
bool MOSTargetLowering::isOffsetFoldingLegal(
...
static void analyzeStandardArguments(TargetLowering::CallLoweringInfo *CLI,
  ...
  static const MCPhysReg RegList8[] = {MOS::R24, MOS::R22, MOS::R20,
                                       MOS::R18, MOS::R16, MOS::R14,
                                       MOS::R12, MOS::R10, MOS::R8};
  static const MCPhysReg RegList16[] = {MOS::R25R24, MOS::R23R22, MOS::R21R20,
                                        MOS::R19R18, MOS::R17R16, MOS::R15R14,
                                        MOS::R13R12, MOS::R11R10, MOS::R9R8};
  if (IsVarArg) {
    // Variadic functions do not need all the analisys below.
    if (IsCall) {
      CCInfo.AnalyzeCallOperands(*Outs, ArgCC_MOS_Vararg);
    } else {
      CCInfo.AnalyzeFormalArguments(*Ins, ArgCC_MOS_Vararg);
    }
    return;
  }
  ...
  for (unsigned i = 0, pos = 0, e = Args.size(); i != e; ++i) {
    ...
    // MOS-GCC does not support zero-sized arguments and so we need not
...
static void analyzeBuiltinArguments(TargetLowering::CallLoweringInfo &CLI,
  ...
  if (FuncName.startswith("__udivmod") || FuncName.startswith("__divmod")) {
    CCInfo.AnalyzeCallOperands(*Outs, ArgCC_MOS_BUILTIN_DIV);
  } else {
...
SDValue MOSTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
                                     SmallVectorImpl<SDValue> &InVals) const {
  ...
  // MOS does not yet support tail call optimization.
  isTailCall = false;
  ...
  if (HasStackArgs) {
    for (AE = AI, AI = ArgLocs.size(); AI != AE; --AI) {
      ...
      SDValue PtrOff = DAG.getNode(
          ...
          DAG.getRegister(MOS::SP, getPointerTy(DAG.getDataLayout())),
  ...
  Chain = DAG.getNode(MOSISD::CALL, DL, NodeTys, Ops);
  InFlag = Chain.getValue(1);
...
///
SDValue MOSTargetLowering::LowerCallResult(
...
bool
MOSTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
...
MachineBasicBlock *MOSTargetLowering::insertShift(MachineInstr &MI,
  ...
  default:
    llvm_unreachable("Invalid shift opcode!");
  case MOS::Lsl8:
    Opc = MOS::ADDRdRr; // LSL is an alias of ADD Rd, Rd
    RC = &MOS::GPR8RegClass;
  ...
  case MOS::Lsr16:
    ...
    break;
  case MOS::Rol8:
    Opc = MOS::ADCRdRr; // ROL is an alias of ADC Rd, Rd
    RC = &MOS::GPR8RegClass;
    HasRepeatedOperand = true;
    break;
  case MOS::Rol16:
  ...
  unsigned ShiftAmtReg = RI.createVirtualRegister(&MOS::LD8RegClass);
  unsigned ShiftAmtReg2 = RI.createVirtualRegister(&MOS::LD8RegClass);
  ...
  // cpi N, 0
  // breq RemBB
  BuildMI(BB, dl, TII.get(MOS::CPIRdK)).addReg(ShiftAmtSrcReg).addImm(0);
  BuildMI(BB, dl, TII.get(MOS::BREQk)).addMBB(RemBB);
  ...
  BuildMI(LoopBB, dl, TII.get(MOS::PHI), ShiftReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(MOS::PHI), ShiftAmtReg)
  ...
  BuildMI(LoopBB, dl, TII.get(MOS::SUBIRdK), ShiftAmtReg2)
      .addReg(ShiftAmtReg)
      .addImm(1);
  BuildMI(LoopBB, dl, TII.get(MOS::BRNEk)).addMBB(LoopBB);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(MOS::PHI), DstReg)
      .addReg(SrcReg)
      .addMBB(BB)
      .addReg(ShiftReg2)
      .addMBB(LoopBB);
...
static bool isCopyMulResult(MachineBasicBlock::iterator const &I) {
  if (I->getOpcode() == MOS::COPY) {
    unsigned SrcReg = I->getOperand(1).getReg();
    return (SrcReg == MOS::R0 || SrcReg == MOS::R1);
  }
...
MachineBasicBlock *MOSTargetLowering::insertMul(MachineInstr &MI,
  ...
  if (isCopyMulResult(I))
    ++I;
  BuildMI(*BB, I, MI.getDebugLoc(), TII.get(MOS::EORRdRr), MOS::R1)
      .addReg(MOS::R1)
      .addReg(MOS::R1);
  return BB;
}
...
MOSTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  assert((Opc == MOS::Select16 || Opc == MOS::Select8) &&
         "Unexpected instr type to insert");

  const MOSInstrInfo &TII = (const MOSInstrInfo &)*MI.getParent()
                                ->getParent()
                                ->getSubtarget()
                                .getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();
  ...
  MOSCC::CondCodes CC = (MOSCC::CondCodes)MI.getOperand(3).getImm();
  BuildMI(MBB, dl, TII.getBrCond(CC)).addMBB(trueMBB);
  BuildMI(MBB, dl, TII.get(MOS::RJMPk)).addMBB(falseMBB);
  MBB->addSuccessor(falseMBB);
  MBB->addSuccessor(trueMBB);

  // Unconditionally flow back to the true block
  BuildMI(falseMBB, dl, TII.get(MOS::RJMPk)).addMBB(trueMBB);
  falseMBB->addSuccessor(trueMBB);

  // Set up the Phi node to determine where we came from
  BuildMI(*trueMBB, trueMBB->begin(), dl, TII.get(MOS::PHI), MI.getOperand(0).getReg())
...
unsigned
MOSTargetLowering::getInlineAsmMemConstraint(StringRef ConstraintCode) const {
...
MOSTargetLowering::ConstraintWeight
MOSTargetLowering::getSingleConstraintMatchWeight(
...
MOSTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r': // Any register: r0..r31.
      if (VT == MVT::i8)
        return std::make_pair(0U, &MOS::GPR8RegClass);
...
void MOSTargetLowering::LowerAsmOperandForConstraint(SDValue Op,
...
unsigned MOSTargetLowering::getRegisterByName(const char *RegName,
                                              EVT VT,
                                              SelectionDAG &DAG) const {
  unsigned Reg;
  ...
  if (VT == MVT::i8) {
    Reg = StringSwitch<unsigned>(RegName)
      .Case("r0", MOS::R0).Case("r1", MOS::R1).Case("r2", MOS::R2)
      .Case("r3", MOS::R3).Case("r4", MOS::R4).Case("r5", MOS::R5)
      .Case("r6", MOS::R6).Case("r7", MOS::R7).Case("r8", MOS::R8)
      .Case("r9", MOS::R9).Case("r10", MOS::R10).Case("r11", MOS::R11)
      .Case("r12", MOS::R12).Case("r13", MOS::R13).Case("r14", MOS::R14)
      .Case("r15", MOS::R15).Case("r16", MOS::R16).Case("r17", MOS::R17)
      .Case("r18", MOS::R18).Case("r19", MOS::R19).Case("r20", MOS::R20)
      .Case("r21", MOS::R21).Case("r22", MOS::R22).Case("r23", MOS::R23)
      .Case("r24", MOS::R24).Case("r25", MOS::R25).Case("r26", MOS::R26)
      .Case("r27", MOS::R27).Case("r28", MOS::R28).Case("r29", MOS::R29)
      .Case("r30", MOS::R30).Case("r31", MOS::R31)
      .Case("X", MOS::R27R26).Case("Y", MOS::R29R28).Case("Z", MOS::R31R30)
      .Default(0);
  } else {
    Reg = StringSwitch<unsigned>(RegName)
      .Case("r0", MOS::R1R0).Case("r2", MOS::R3R2)
      .Case("r4", MOS::R5R4).Case("r6", MOS::R7R6)
      .Case("r8", MOS::R9R8).Case("r10", MOS::R11R10)
      .Case("r12", MOS::R13R12).Case("r14", MOS::R15R14)
      .Case("r16", MOS::R17R16).Case("r18", MOS::R19R18)
      .Case("r20", MOS::R21R20).Case("r22", MOS::R23R22)
      .Case("r24", MOS::R25R24).Case("r26", MOS::R27R26)
      .Case("r28", MOS::R29R28).Case("r30", MOS::R31R30)
      .Case("X", MOS::R27R26).Case("Y", MOS::R29R28).Case("Z", MOS::R31R30)
      .Default(0);
  }
