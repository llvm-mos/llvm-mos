...
bool MOSRegisterInfo::scavengingRegHelpsScavengeClass(
    Register Reg, const TargetRegisterClass *RC) const {
  if (RC->contains(Reg))
    return true;
  return Reg == MOS::A && (RC->contains(MOS::X) || RC->contains(MOS::Y));
}
