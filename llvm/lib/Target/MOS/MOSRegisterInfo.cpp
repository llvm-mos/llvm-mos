...
BitVector MOSRegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  Reserved.set(MOS::SPL);
  Reserved.set(MOS::SPH);
  Reserved.set(MOS::SP);
...
MOSRegisterInfo::getLargestLegalSuperClass(const TargetRegisterClass *RC,
  ...
  if (TRI->isTypeLegalForClass(*RC, MVT::i16)) {
    return &MOS::DREGSRegClass;
  }

  if (TRI->isTypeLegalForClass(*RC, MVT::i8)) {
    return &MOS::GPR8RegClass;
  }
...
void MOSRegisterInfo::eliminateFrameIndex(MachineBasicBlock::iterator II,
  ...
  const MOSTargetMachine &TM = (const MOSTargetMachine &)MF.getTarget();
  ...
  if (MI.getOpcode() == MOS::FRMIDX) {
    MI.setDesc(TII.get(MOS::MOVWRdRr));
    ...
    assert(DstReg != MOS::R29R28 && "Dest reg cannot be the frame pointer");
...
void MOSRegisterInfo::splitReg(unsigned Reg,
    ...
    assert(MOS::DREGSRegClass.contains(Reg) && "can only split 16-bit registers");

    LoReg = getSubReg(Reg, MOS::sub_lo);
    HiReg = getSubReg(Reg, MOS::sub_hi);
}
