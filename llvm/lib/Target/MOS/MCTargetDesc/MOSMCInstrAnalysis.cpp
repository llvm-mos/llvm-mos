...
bool MOSMCInstrAnalysis::evaluateBranch(const MCInst &Inst, uint64_t Addr,
  ...
  case MOSOp::OPERAND_ADDR24: {
    Target =
        (Addr & 0xFF000000) | (Inst.getOperand(NumOps - 1).getImm() & 0xFFFFFF);
    return true;
  }
...
std::optional<uint64_t> MOSMCInstrAnalysis::evaluateMemoryOperandAddress(
    const MCInst &Inst, const MCSubtargetInfo *STI, uint64_t Addr,
    uint64_t Size) const {
  uint64_t ZpAddrOffset = STI->hasFeature(MOS::FeatureHUC6280) ? 0x2000 : 0;
  uint64_t AbsAddrMask =
      STI->hasFeature(MOS::FeatureW65816) ? 0xFFFFFF : 0xFFFF;
  ...
  for (unsigned OpIdx = 0; OpIdx < NumOps; OpIdx++) {
    ...
    case MOSOp::OPERAND_ADDR13: {
      return (Addr & ~AbsAddrMask) | (Inst.getOperand(OpIdx).getImm() & 0x1FFF);
    }
    case MOSOp::OPERAND_ADDR16: {
