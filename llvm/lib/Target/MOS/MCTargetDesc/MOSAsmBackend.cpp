...
static int getRelativeMOSPCCorrection(const bool IsPCRel16,
                                      const bool Is65CE02) {
  // LLVM measures PC-relative fixups from the fixup location, i.e. opcode + 1.
  // 65816 BRL is relative to the end of the instruction, opcode + 3; every
  // other branch, including the 65CE02 16-bit ones, is relative to opcode + 2.
  return IsPCRel16 && !Is65CE02 ? -2 : -1;
}
