...
bool MOSAsmBackend::fixupNeedsRelaxationAdvanced(const MCFixup &Fixup,
  ...
  if (dyn_cast<MOSMCExpr>(Fixup.getValue()) != nullptr) {
    return (Info.TargetSize > (BankRelax ? 16 : 8));
  }
  // Now the fixup kind is not target-specific.
  // If it requires more than 8 (or 16) bits, then relaxation is needed.
...
bool MOSAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
  ...
  while ((Count--) > 0) {
    OS << 0xEA; // Sports. It's in the game.  Knowing the hexadecimal
