...
void adjustBranch(unsigned Size, const MCFixup &Fixup, uint64_t &Value,
  ...
  MOS::fixups::adjustBranchTarget(Value);
}
...
MOSAsmBackend::createObjectTargetWriter() const {
  return createMOSELFObjectWriter(MCELFObjectTargetWriter::getOSABI(OSType));
}
...
MCFixupKindInfo const &MOSAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  // NOTE: Many MOS fixups work on sets of non-contignous bits. We work around
...
bool MOSAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                          const MCFixup &Fixup,
                                          const MCValue &Target) {
