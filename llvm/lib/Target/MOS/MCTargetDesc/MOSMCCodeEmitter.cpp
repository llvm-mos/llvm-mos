...
unsigned
MOSMCCodeEmitter::loadStorePostEncoder(const MCInst &MI, unsigned EncodedValue,
...
unsigned
MOSMCCodeEmitter::encodeRelCondBrTarget(const MCInst &MI, unsigned OpNo,
  ...
  MOS::fixups::adjustBranchTarget(target);
  return target;
}

unsigned MOSMCCodeEmitter::encodeLDSTPtrReg(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  ...
  switch (MO.getReg()) {
  case MOS::R27R26: return 0x03; // X: 0b11
  case MOS::R29R28: return 0x02; // Y: 0b10
  case MOS::R31R30: return 0x00; // Z: 0b00
  default:
...
unsigned MOSMCCodeEmitter::encodeMemri(const MCInst &MI, unsigned OpNo,
                                       SmallVectorImpl<MCFixup> &Fixups,
                                       const MCSubtargetInfo &STI) const {
  ...
  if (OffsetOp.isImm()) {
    ...
    Fixups.push_back(MCFixup::create(0, OffsetOp.getExpr(),
                     MCFixupKind(MOS::fixup_6), MI.getLoc()));
  } else {
...
unsigned MOSMCCodeEmitter::encodeComplement(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
...
unsigned MOSMCCodeEmitter::encodeImm(const MCInst &MI, unsigned OpNo,
  ...
  if (MO.isExpr()) {
    if (isa<MOSMCExpr>(MO.getExpr())) {
      // If the expression is already an MOSMCExpr (i.e. a lo8(symbol),
...
unsigned MOSMCCodeEmitter::encodeCallTarget(const MCInst &MI, unsigned OpNo,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  auto MO = MI.getOperand(OpNo);

  if (MO.isExpr()) {
    MCFixupKind FixupKind = static_cast<MCFixupKind>(MOS::fixup_call);
