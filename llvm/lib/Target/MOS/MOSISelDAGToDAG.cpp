...
bool MOSDAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
  Subtarget = &MF.getSubtarget<MOSSubtarget>();
  return SelectionDAGISel::runOnMachineFunction(MF);
}

bool MOSDAGToDAGISel::SelectAddr(SDNode *Op, SDValue N, SDValue &Base,
                                 SDValue &Disp) {
  SDLoc dl(Op);
...
bool MOSDAGToDAGISel::selectIndexedLoad(SDNode *N) {
...
unsigned MOSDAGToDAGISel::selectIndexedProgMemLoad(const LoadSDNode *LD,
                                                   MVT VT) {
  ISD::MemIndexedMode AM = LD->getAddressingMode();
...
bool MOSDAGToDAGISel::SelectInlineAsmMemoryOperand(const SDValue &Op,
...
template <> bool MOSDAGToDAGISel::select<ISD::FrameIndex>(SDNode *N) {
  auto DL = CurDAG->getDataLayout();
  ...
  CurDAG->SelectNodeTo(N, MOS::FRMIDX,
                       getTargetLowering()->getPointerTy(DL), TFI,
                       CurDAG->getTargetConstant(0, SDLoc(N), MVT::i16));
  return true;
}

template <> bool MOSDAGToDAGISel::select<ISD::STORE>(SDNode *N) {
  ...
  unsigned Opc = (VT == MVT::i16) ? MOS::STDWSPQRr : MOS::STDSPQRr;
...
template <> bool MOSDAGToDAGISel::select<ISD::LOAD>(SDNode *N) {
  const LoadSDNode *LD = cast<LoadSDNode>(N);
  ...
  Chain = CurDAG->getCopyToReg(Chain, DL, MOS::R31R30, Ptr, SDValue());
  Ptr = CurDAG->getCopyFromReg(Chain, DL, MOS::R31R30, MVT::i16,
                               Chain.getValue(1));

  SDValue RegZ = CurDAG->getRegister(MOS::R31R30, MVT::i16);
...
template <> bool MOSDAGToDAGISel::select<MOSISD::CALL>(SDNode *N) {
  SDValue InFlag;
  ...
  SDLoc DL(N);
  Chain = CurDAG->getCopyToReg(Chain, DL, MOS::R31R30, Callee, InFlag);
  SmallVector<SDValue, 8> Ops;
  Ops.push_back(CurDAG->getRegister(MOS::R31R30, MVT::i16));
  ...
  SDNode *ResNode =
    CurDAG->getMachineNode(MOS::ICALL, DL, MVT::Other, MVT::Glue, Ops);
...
template <> bool MOSDAGToDAGISel::select<ISD::BRIND>(SDNode *N) {
  SDValue Chain = N->getOperand(0);
  SDValue JmpAddr = N->getOperand(1);

  SDLoc DL(N);
  // Move the destination address of the indirect branch into R31R30.
  Chain = CurDAG->getCopyToReg(Chain, DL, MOS::R31R30, JmpAddr);
  SDNode *ResNode = CurDAG->getMachineNode(MOS::IJMP, DL, MVT::Other, Chain);
...
bool MOSDAGToDAGISel::selectMultiplication(llvm::SDNode *N) {
  SDLoc DL(N);
  ...
  unsigned MachineOp = isSigned ? MOS::MULSRdRr : MOS::MULRdRr;
  ...
  if (N->hasAnyUseOfValue(0)) {
    SDValue CopyFromLo =
        CurDAG->getCopyFromReg(InChain, DL, MOS::R0, Type, InGlue);
  ...
  if (N->hasAnyUseOfValue(1)) {
    SDValue CopyFromHi =
        CurDAG->getCopyFromReg(InChain, DL, MOS::R1, Type, InGlue);
...
bool MOSDAGToDAGISel::trySelect(SDNode *N) {
  ...
  case MOSISD::CALL: return select<MOSISD::CALL>(N);
