...
Instruction *InstCombinerImpl::visitInsertElementInst(InsertElementInst &IE) {
  ...
  if (auto *IndexC = dyn_cast<ConstantInt>(IdxOp)) {
    ...
    if (match(VecOp,
              m_OneUse(m_InsertElt(m_Value(BaseVec), m_Value(OtherScalar),
...
static Value *buildNew(Instruction *I, ArrayRef<Value *> NewOps,
  ...
  case Instruction::GetElementPtr: {
    ...
    return Builder.CreateGEP(cast<GEPOperator>(I)->getSourceElementType(), Ptr,
                             Idx, "", cast<GEPOperator>(I)->getNoWrapFlags());
  }
  }
...
static Value *evaluateInDifferentElementOrder(Value *V, ArrayRef<int> Mask,
  ...
  case Instruction::InsertElement: {
    ...
    Value *V = evaluateInDifferentElementOrder(I->getOperand(0), Mask, Builder);
    Builder.SetInsertPoint(I);
